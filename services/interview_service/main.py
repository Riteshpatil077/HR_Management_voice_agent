"""
Interview Service Entrypoint (FastAPI).
...
from services.interview_service.api.router import router as interview_router
...
logger = structlog.get_logger("interview_service")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("interview_service_shutting_down")
...
app = FastAPI(
    title="HR Voice Agent - Interview Service",
...
# Middleware
app.add_middleware(MetricsMiddleware, service_name="interview_service")
...
async def health_check() -> JSONResponse:
    """Liveness probe."""
    return JSONResponse({"status": "ok", "service": "interview_service"})

@app.get("/readiness", tags=["System"])
async def readiness_check() -> JSONResponse:
    """Readiness probe."""
    return JSONResponse({"status": "ready", "service": "interview_service"})
