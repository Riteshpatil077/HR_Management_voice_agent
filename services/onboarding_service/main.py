"""
Onboarding Service Entrypoint (FastAPI).
...
from services.onboarding_service.api.router import router as onboarding_router
...
logger = structlog.get_logger("onboarding_service")
settings = get_settings()

@asynccontextmanager
async def lifespan(app: FastAPI):
    """Lifecycle events for the Onboarding Service."""
    logger.info("onboarding_service_starting", version=settings.app_version)
...
app = FastAPI(
    title="HR Voice Agent - Onboarding Service",
...
# Middleware
app.add_middleware(MetricsMiddleware, service_name="onboarding_service")
...
async def health_check() -> JSONResponse:
    """Liveness probe."""
    return JSONResponse({"status": "ok", "service": "onboarding_service"})

@app.get("/readiness", tags=["System"])
async def readiness_check() -> JSONResponse:
    """Readiness probe."""
    return JSONResponse({"status": "ready", "service": "onboarding_service"})
